...
const filterStringsWithVowels = (arr) => {
  // Solution code here...
  var result = /[AEIOUaeiou]/g; 
  let x = arr.filter(y =>{
    if (!!y.match(result) == true){
        return y;
    }
  }); 
  return x ;
...
const notInFirstArray = (forbiddenValues, arr) => {
  ...
  let test = [];
  arr.filter(i =>{
    if(forbiddenValues.includes(i) === true){
      return i;
    }
    else if(forbiddenValues.includes(i) == false){
        test.push(i);
    }
  })
  return test;
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  // Solution code here...
  let test = arr.filter(i =>{
    if ((i.baseStat > minBaseStat)===true){
      return i ;
    }
  }); 
  return test;
